...
mod dlt_bindings;
use dlt_bindings::{DLT_ID_SIZE, DltContext, DltContextData};
...
impl From<i32> for DltLogLevel {
    fn from(value: i32) -> Self {
        match value {
            dlt_bindings::DltLogLevelType_DLT_LOG_OFF => DltLogLevel::Off,
            dlt_bindings::DltLogLevelType_DLT_LOG_FATAL => DltLogLevel::Fatal,
            dlt_bindings::DltLogLevelType_DLT_LOG_ERROR => DltLogLevel::Error,
            dlt_bindings::DltLogLevelType_DLT_LOG_WARN => DltLogLevel::Warn,
            dlt_bindings::DltLogLevelType_DLT_LOG_INFO => DltLogLevel::Info,
            dlt_bindings::DltLogLevelType_DLT_LOG_DEBUG => DltLogLevel::Debug,
            dlt_bindings::DltLogLevelType_DLT_LOG_VERBOSE => DltLogLevel::Verbose,
...
impl From<DltLogLevel> for i32 {
    fn from(value: DltLogLevel) -> Self {
        match value {
            DltLogLevel::Default => dlt_bindings::DltLogLevelType_DLT_LOG_DEFAULT,
            DltLogLevel::Off => dlt_bindings::DltLogLevelType_DLT_LOG_OFF,
            DltLogLevel::Fatal => dlt_bindings::DltLogLevelType_DLT_LOG_FATAL,
            DltLogLevel::Error => dlt_bindings::DltLogLevelType_DLT_LOG_ERROR,
            DltLogLevel::Warn => dlt_bindings::DltLogLevelType_DLT_LOG_WARN,
            DltLogLevel::Info => dlt_bindings::DltLogLevelType_DLT_LOG_INFO,
            DltLogLevel::Debug => dlt_bindings::DltLogLevelType_DLT_LOG_DEBUG,
            DltLogLevel::Verbose => dlt_bindings::DltLogLevelType_DLT_LOG_VERBOSE,
        }
    }
}
...
impl DltSysError {
    fn from_return_code(code: i32) -> Result<(), Self> {
        ...
        match code {
            dlt_bindings::DltReturnValue_DLT_RETURN_TRUE
            | dlt_bindings::DltReturnValue_DLT_RETURN_OK => Ok(()),
            dlt_bindings::DltReturnValue_DLT_RETURN_ERROR => Err(DltSysError::Error),
            dlt_bindings::DltReturnValue_DLT_RETURN_PIPE_ERROR => Err(DltSysError::PipeError),
            dlt_bindings::DltReturnValue_DLT_RETURN_PIPE_FULL => Err(DltSysError::PipeFull),
            dlt_bindings::DltReturnValue_DLT_RETURN_BUFFER_FULL => Err(DltSysError::BufferFull),
            dlt_bindings::DltReturnValue_DLT_RETURN_WRONG_PARAMETER => {
                Err(DltSysError::WrongParameter)
            }
            dlt_bindings::DltReturnValue_DLT_RETURN_USER_BUFFER_FULL => {
                Err(DltSysError::UserBufferFull)
            }
            dlt_bindings::DltReturnValue_DLT_RETURN_LOGGING_DISABLED => {
                Err(DltSysError::LoggingDisabled)
            }
            dlt_bindings::DltReturnValue_DLT_RETURN_FILESZERR => Err(DltSysError::FileSizeError),
            #[cfg(feature = "trace_load_ctrl")]
            dlt_bindings::DltReturnValue_DLT_RETURN_LOAD_EXCEEDED => Err(DltSysError::LoadExceeded),
...
impl From<i32> for DltTraceStatus {
    fn from(value: i32) -> Self {
        match value {
            dlt_bindings::DltTraceStatusType_DLT_TRACE_STATUS_OFF => DltTraceStatus::Off,
            dlt_bindings::DltTraceStatusType_DLT_TRACE_STATUS_ON => DltTraceStatus::On,
...
impl From<DltTraceStatus> for i32 {
    fn from(value: DltTraceStatus) -> Self {
        match value {
            DltTraceStatus::Default => dlt_bindings::DltTraceStatusType_DLT_TRACE_STATUS_DEFAULT,
            DltTraceStatus::Off => dlt_bindings::DltTraceStatusType_DLT_TRACE_STATUS_OFF,
            DltTraceStatus::On => dlt_bindings::DltTraceStatusType_DLT_TRACE_STATUS_ON,
        }
    }
}
...
impl Drop for DltApplicationHandle {
    fn drop(&mut self) {
        unsafe {
            ...
            dlt_bindings::unregisterApplicationFlushBufferedLogs();
            dlt_bindings::dltFree();
...
impl DltApplication {
    ...
    pub fn register(app_id: &DltId, app_description: &str) -> Result<Self, DltError> {
        ...
        unsafe {
            let ret = dlt_bindings::registerApplication(app_id_c.as_ptr(), app_desc_c.as_ptr());
...
impl DltContextHandle {
    ...
    fn new(
        ...
        unsafe {
            let context = dlt_bindings::registerContext(ctx_id_c.as_ptr(), ctx_desc_c.as_ptr());
            if context.is_null() {
    ...
    pub fn log(&self, log_level: DltLogLevel, message: &str) -> Result<(), DltSysError> {
        ...
        unsafe {
            let ret = dlt_bindings::logDlt(self.context, log_level.into(), msg_c.as_ptr());
    ...
    pub fn log_write_start_custom_timestamp(
        ...
        log_writer.log_data.use_timestamp = dlt_bindings::DltTimestampType_DLT_USER_TIMESTAMP;
    ...
    pub fn log_write_start(&self, log_level: DltLogLevel) -> Result<DltLogWriter, DltSysError> {
        ...
        unsafe {
            let ret = dlt_bindings::dltUserLogWriteStart(
                self.context,
                &raw mut log_data,
                log_level.into(),
            );
    ...
    pub fn register_log_level_changed_listener(
        ...
        if let Some(broadcaster) = guard.get_mut(&ctx_id) {
            ...
            unsafe {
                let ret = dlt_bindings::registerLogLevelChangedCallback(
                    self.context,
...
impl Drop for DltContextHandle {
    fn drop(&mut self) {
        ...
        unsafe {
            dlt_bindings::unregisterContext(self.context);
        }
    }
}
...
impl DltLogWriter {
    ...
    pub fn write_string(&mut self, text: &str) -> Result<&mut Self, DltSysError> {
        ...
        unsafe {
            let ret = dlt_bindings::dltUserLogWriteString(&raw mut self.log_data, text_c.as_ptr());
            DltSysError::from_return_code(ret)?;
        }
    ...
    pub fn write_u32(&mut self, value: u32) -> Result<&mut Self, DltSysError> {
        unsafe {
            let ret = dlt_bindings::dltUserLogWriteUint(&raw mut self.log_data, value);
            DltSysError::from_return_code(ret)?;
        }
    ...
    pub fn write_i32(&mut self, value: i32) -> Result<&mut Self, DltSysError> {
        unsafe {
            let ret = dlt_bindings::dltUserLogWriteInt(&raw mut self.log_data, value);
            DltSysError::from_return_code(ret)?;
        }
    ...
    pub fn write_uint64(&mut self, value: u64) -> Result<&mut Self, DltSysError> {
        unsafe {
            let ret = dlt_bindings::dltUserLogWriteUint64(&raw mut self.log_data, value);
            DltSysError::from_return_code(ret)?;
        }
    ...
    pub fn write_int64(&mut self, value: i64) -> Result<&mut Self, DltSysError> {
        unsafe {
            let ret = dlt_bindings::dltUserLogWriteInt64(&raw mut self.log_data, value);
            DltSysError::from_return_code(ret)?;
        }
    ...
    pub fn write_float32(&mut self, value: f32) -> Result<&mut Self, DltSysError> {
        unsafe {
            let ret = dlt_bindings::dltUserLogWriteFloat32(&raw mut self.log_data, value);
            DltSysError::from_return_code(ret)?;
        }
    ...
    pub fn write_float64(&mut self, value: f64) -> Result<&mut Self, DltSysError> {
        unsafe {
            let ret = dlt_bindings::dltUserLogWriteFloat64(&raw mut self.log_data, value);
            DltSysError::from_return_code(ret)?;
        }
    ...
    pub fn write_bool(&mut self, value: bool) -> Result<&mut Self, DltSysError> {
        unsafe {
            let ret = dlt_bindings::dltUserLogWriteBool(&raw mut self.log_data, u8::from(value));
            DltSysError::from_return_code(ret)?;
        }
    ...
    pub fn finish(mut self) -> Result<(), DltSysError> {
        let ret = unsafe { dlt_bindings::dltUserLogWriteFinish(&raw mut self.log_data) };
...
impl Drop for DltLogWriter {
    fn drop(&mut self) {
        ...
        unsafe {
            let _ = dlt_bindings::dltUserLogWriteFinish(&raw mut self.log_data);
        }
    }
}
